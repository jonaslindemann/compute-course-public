...
class AnimationCanvas(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        for i in range(50):
            ...
            picked_shape = random.choice(range(3))

            if picked_shape == 0:
                self.shapes.append(RectangleShape(x, y, w, h, color))
                self.add_widget(self.shapes[-1])

            if picked_shape == 1:
                self.shapes.append(CircleShape(x, y, radius, color))
                self.add_widget(self.shapes[-1])

            if picked_shape == 2:
                self.shapes.append(TriangleShape(x, y, w, h, color))
                self.add_widget(self.shapes[-1])
